...
export default function ErrorBoundary({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => { console.error("[Skills Error]", error); }, [error]);
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] p-8 text-center">
      ...
      <h2 className="text-xl font-bold text-[hsl(var(--foreground))] mb-2">Couldn&apos;t load skills</h2>
      <p className="text-sm text-[hsl(var(--muted-foreground))] max-w-sm mb-6">{error.message || "Something went wrong computing your skill data."}</p>
